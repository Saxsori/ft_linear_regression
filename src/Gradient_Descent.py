...
class GradientDescent :
    def __init__(self, xs: np.ndarray, ys: np.ndarray, lr: Decimal) -> None:
        ...
        self._default_mse = Decimal('1e-6')
    
    @staticmethod
    def estimate(theta0: Decimal, theta1: Decimal, x: Decimal) -> Decimal :
    ...
    def train_model(self) -> list:
        ...
        per_mse = None
        for (i) in range(1000):
            tempTheta0 = self.compute_theta0(theta0, theta1)
            tempTheta1 = self.compute_theta1(theta0, theta1)
            theta0 -= tempTheta0
            theta1 -= tempTheta1
...
def testGradientDescent () :
	...
	gd = GradientDescent(xs, ys, Decimal('0.000000000000000000000000000008'))
